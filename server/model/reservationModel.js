...
Reservation.saveReservation = (newReservation, result) => {
  sql.query(
    `SELECT rsvvisitorid, rsvdtstart FROM dbparkr.reservations WHERE rsvvisitorid= ? AND rsvdtstart =  ?`,
    [newReservation.rsvvisitorid, newReservation.rsvdtstart],
    (err, res) => {
      if (err) {
  ...
  sql.query(
    "INSERT INTO dbparkr.reservations SET ?",
    newReservation,
    (err, res) => {
      if (err) {
        console.log("error: ", err);
...
Reservation.setReservationToCancel = (bookingId, result) => {
  sql.query(
    "UPDATE dbparkr.reservations SET rsvstatus = 0 WHERE idbooking = ?",
    bookingId,
    (err, res) => {
      if (err) {
        console.log("error: ", err);
      ...
      console.log(`reservation id ${bookingId} has been cancelled.`, {
        id: bookingId,
      });
